<tr>
    <td>
        <div class="id-order">{{ $product->id }}</div>
    </td>
    <td>
        <p class="m-b-5">{{ $product->name }}</p>
    </td>
    <td>
        ...
        <p class="m-b-0" style="white-space: normal">Mô tả ngắn: {{ $product->description ?? 'Chưa có' }}</p>
    </td>
    <td>
        @php
        $images = $product->images && $product->images !== '[]'
        ? array_filter(array_map('trim', explode(',', $product->images)))
        : [];
        @endphp
        @if (count($images))
    ...
    <td>
        <div class="view-data" style="cursor:pointer">
            <a href="#" class="btn btn-default btn-sm p-0 border-0 bg-transparent"
                ...
                data-id="{{ $product->id }}"><i class="feather-edit"></i></a>
        </div>
        <form action="{{ route('admin.product.delete', $product->id) }}" method="POST" class="d-inline view-data" onsubmit="return confirm('Xóa sản phẩm và tất cả biến thể?')">
            @csrf
